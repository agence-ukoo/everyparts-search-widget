...
| `product_click` | Clic sur une carte produit — clic normal (`click`) ou clic milieu ouvrant un nouvel onglet (`auxclick`, `button === 1`). Un clic droit → « Ouvrir dans un nouvel onglet » depuis le menu contextuel, en revanche, ne déclenche aucun événement de la famille click côté page — hors de portée de tout écouteur JS, quel que soit le site. | `product_ref` *(requis)*, `position`, `page`, `query`, `interpreted`, `model_confirmed`, `result_count` (= `pagination.total`), `price`, `currency`, `name`, `brand`, `url` |
| `samples_click` | Clic sur une suggestion « Essayez » de l'accueil | `sample` *(requis)*, `position`, `sample_count` |
| `contact_click` | Clic sur un lien de contact — celui de l'accueil (`welcome_p3`) comme celui d'une balise `[link]` d'une réponse. Écoute déléguée sur le fil (`click` + `auxclick`, même raison que `product_click`) : un lien à venir n'a rien à déclarer pour être compté. **Vidé tout de suite**, comme `product_click` : le lien peut s'ouvrir dans le même onglet, la file n'aurait jamais le temps de partir. | `url` *(requis)* — celle réellement ouverte : le `contact_url` de la réponse s'il y en a un, sinon `data-contact-page-url` |
