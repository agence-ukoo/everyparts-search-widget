(function(){"use strict";const f=document.currentScript||(function(){const a=document.querySelectorAll("script[data-token]");return a[a.length-1]})(),u={token:f?.getAttribute("data-token")||"",lang:f?.getAttribute("data-lang")||"fr",position:f?.getAttribute("data-position")||"bottom-right",apiBase:f?.getAttribute("data-api")||"http://localhost:8000/api/v1"},M={fr:{placeholder:"Rechercher une pièce compatible…",send:"Envoyer",welcome:"Bonjour ! Je suis PartsMind, l'assistant de recherche EveryParts. Décrivez-moi votre moto et la pièce que vous cherchez (ex : bougie pour Honda CBR 600 de 96).",typing:"En train de répondre…",error_token:"Configuration invalide : token absent.",error_net:"Impossible de joindre l'API. Vérifiez votre connexion.",compat_label:"Compatibilité confirmée",compat_tip:"Confirmée par {n} sources",no_results_title:"Aucun résultat",suggestions:"Suggestions :",close:"Fermer le chat",open:"Ouvrir l'assistant EveryParts",price_unit:"€"}};function o(a,d={}){let i=(M[u.lang]||M.fr)[a]||a;for(const[r,p]of Object.entries(d))i=i.replace(`{${r}}`,p);return i}const A=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 271 281" width="32" height="32">
  ...
  </svg>`,N=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 1080 208" width="140" height="27" style="enable-background:new 0 0 1080 208;" xml:space="preserve">
  ...
  `;let H=q(),x={previous_clarifications:[]},m=!1,y=null;function q(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,a=>{const d=Math.random()*16|0;return(a==="x"?d:d&3|8).toString(16)})}function z(){const a=document.createElement("div");a.id="everyparts-widget-host",document.body.appendChild(a);const d=a.attachShadow({mode:"open"}),w=document.createElement("style");w.textContent=B,d.appendChild(w);const i=document.createElement("button");i.id="ep-fab",i.className=`ep-pos-${u.position}`,i.setAttribute("aria-label",o("open")),i.title=o("open"),i.innerHTML=A,d.appendChild(i);const r=document.createElement("div");r.id="ep-window",r.className=`ep-pos-${u.position} ep-hidden`,r.setAttribute("role","dialog"),r.setAttribute("aria-label","EveryParts – Assistant de recherche"),r.innerHTML=T(),d.appendChild(r);const p=r.querySelector("#ep-messages"),j=r.querySelector("#ep-typing"),b=r.querySelector("#ep-input"),C=r.querySelector("#ep-send-btn"),D=r.querySelector("#ep-close-btn");let F=!1;i.addEventListener("click",()=>k(!0)),D.addEventListener("click",()=>k(!1)),d.addEventListener("keydown",e=>{e.key==="Escape"&&F&&k(!1)}),C.addEventListener("click",$),b.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),$())});function k(e){F=e,r.classList.toggle("ep-hidden",!e),i.setAttribute("aria-label",o(e?"close":"open")),e&&(b.focus(),p.children.length===0&&h(o("welcome")))}function $(){const e=b.value.trim();!e||m||(b.value="",S(e),_(e))}async function _(e){if(m)return;m=!0,C.disabled=!0,L(!0);const t={query:e,session_id:H,context:x};try{const n=await(await fetch(`${u.apiBase}/search`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${u.token}`},body:JSON.stringify(t)})).json();L(!1),I(n)}catch{L(!1),E(o("error_net"))}finally{m=!1,C.disabled=!1,b.focus()}}function I(e){switch(e.type){case"results":V(e);break;case"clarification":O(e);break;case"no_results":P(e);break;case"error":E(e.error?.message||"Erreur inconnue.");break;default:E("Réponse inattendue du serveur.")}}function V(e){if(h(e.message),e.results&&e.results.length>0){const t=document.createElement("div");t.className="ep-cards",e.results.forEach(n=>{const l=document.createElement("div");l.className="ep-card";const v=n.compatibility?.n_sources||0,Y=v>=5?o("compat_tip",{n:v}):o("compat_label");l.innerHTML=`
            ...
            <div class="ep-card-body">
              <div class="ep-card-brand">${c(n.brand)}</div>
              <div class="ep-card-name"><a href="${c(n.url)}">${c(n.name)}</a></div>
              <sub class="ep-card-ref">${c(n.product_ref)}</sub>
              <div class="ep-card-price">${c(String(n.price))} ${o("price_unit")}</div>
              <span class="ep-badge" title="${c(Y)}">${c(n.compatibility?.label||o("compat_label"))}</span>
            </div>
          `,t.appendChild(l)});const s=document.createElement("div");s.className="ep-msg ep-msg-assistant",s.appendChild(t),p.appendChild(s),g()}x={previous_clarifications:[]},y=null}function O(e){if(h(e.message),y=e.clarification?.field||null,e.clarification?.options?.length>0){const t=document.createElement("div");t.className="ep-clari-opts",e.clarification.options.forEach(n=>{const l=document.createElement("button");l.className="ep-clari-btn",l.textContent=n,l.addEventListener("click",()=>{x.previous_clarifications.push({field:y,answer:n}),S(n),_(n),t.querySelectorAll(".ep-clari-btn").forEach(v=>v.disabled=!0)}),t.appendChild(l)});const s=document.createElement("div");s.className="ep-msg ep-msg-assistant",s.appendChild(t),p.appendChild(s),g()}}function P(e){const t=document.createElement("div"),s=document.createElement("div");if(s.textContent=e.message,e.suggestions?.length>0){const n=document.createElement("div");n.className="ep-suggestions",n.innerHTML=`<p>${o("suggestions")}</p><ul>${e.suggestions.map(l=>`<li>${c(l)}</li>`).join("")}</ul>`,t.appendChild(s),t.appendChild(n),R(t)}else h(e.message);x={previous_clarifications:[]}}function S(e){const t=document.createElement("div");t.className="ep-msg ep-msg-user";const s=document.createElement("div");s.className="ep-bubble",s.textContent=e,t.appendChild(s),p.appendChild(t),g()}function h(e){const t=document.createElement("div");t.className="ep-msg ep-msg-assistant";const s=document.createElement("div");s.className="ep-bubble",s.textContent=e,t.appendChild(s),p.appendChild(t),g()}function R(e){const t=document.createElement("div");t.className="ep-msg ep-msg-assistant";const s=document.createElement("div");s.className="ep-bubble",s.appendChild(e),t.appendChild(s),p.appendChild(t),g()}function E(e){const t=document.createElement("div");t.className="ep-msg ep-msg-assistant ep-msg-error";const s=document.createElement("div");s.className="ep-bubble",s.textContent=e,t.appendChild(s),p.appendChild(t),g()}function L(e){j.classList.toggle("ep-visible",e),e&&g()}function g(){requestAnimationFrame(()=>{p.scrollTop=p.scrollHeight})}}function T(){return`
      <div id="ep-header">
        <div id="ep-header-logo">
          ${N}
        </div>
        <button id="ep-close-btn" aria-label="${o("close")}" title="${o("close")}">
      ...
      <div id="ep-input-area">
        <input
          ...
          maxlength="300"
        />
        <button id="ep-send-btn" aria-label="${o("send")}">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    `}function c(a){return typeof a!="string"&&(a=String(a??"")),a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}u.token||console.error("[EveryParts Widget] data-token manquant sur la balise <script>."),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",z):z()})();
