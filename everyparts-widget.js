...
(function () {
  ...
  // Avatar de l'assistant, servi par la boutique. Une ADRESSE, jamais du
  // balisage : un logo de marchand est multi-tracés et multi-couleurs, une
  // donnée de tracé ne saurait pas le porter. Rendu en <img>, comme data-logo.
  const REMOTE_AVATAR = typeof REMOTE_CONFIG.avatar === 'string' ? REMOTE_CONFIG.avatar : '';
  ...
  const I18N = {
    'fr-FR': {
      ...
      see_all_parts:   'Toutes les pièces',
      ...
      model_no_parts:          'Nous n\'avons aucune pièce référencée pour ce modèle. Un expert peut la rechercher pour vous.',
    ...
    'en-US': {
      ...
      model_no_parts:          'We have no parts listed for this model. An expert can look one up for you.',
  ...
  const CONFIG = {
    ...
    icon: REMOTE_ICON,
    // Avatar des messages de l'assistant (cf. REMOTE_AVATAR). Vide = la marque
    // EveryParts.
  ...
  // Peint l'avatar d'un message de l'assistant : le logo de la boutique quand
  // elle en sert un, la marque EveryParts sinon.
  //
  // Le BOUTON FLOTTANT n'en dépend pas et garde la marque : il annonce le
  // produit, pas le marchand. L'en-tête non plus — c'est `data-logo` qui le
  // gouverne, et il le faisait déjà.
  function paintAvatar(el) {
    el.className = 'ep-msg-avatar';
    el.setAttribute('aria-hidden', 'true');

    if (CONFIG.avatar) {
      el.classList.add('ep-avatar-custom');
      el.innerHTML = `<img src="${escHtml(CONFIG.avatar)}" alt="">`;
      return;
    }
  ...
  const STYLES = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Avatar servi par la boutique : l'image occupe la pastille, et le fond
       sombre disparaît — il n'existe que pour faire ressortir la marque
       EveryParts, et masquerait un logo clair. */
    .ep-msg-avatar.ep-avatar-custom { background: none; }
    .ep-msg-avatar img { width: 100%; height: 100%; object-fit: contain; border-radius: inherit; display: block; }
  ...
  function mount() {
    ...
    function renderClarification(data) {
      ...
      appendOptionsGroup(options, value => {
        ...
        if (withoutParts.indexOf(value) !== -1 && CONFIG.partsRequest) {
          openPartsRequestForModel(value, data);
          return;
        }
    ...
    // faire relire la même absence, un tour plus tard : on ouvre la demande de
    // pièce, en lui disant d'abord pourquoi.
    //
    ...
    function openPartsRequestForModel(label, data) {
      ...
      openPartsRequest({
        ...
        vehicle,
        part: identifiedPart,
        query: interpreted.part_type || lastUserQuery(),
        otherParts: [],
        otherPagination: null,
      }, null, 'model_without_parts');
    }
